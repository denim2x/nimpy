...
import oswalkdir, os, strutils, strformat
...
proc calcPythonExecutables() : seq[string] =
  ...
  ##
  ## `NIMPY_PY_EXES=&"python2:/usr/local/bin/python3.7{sep}/usr/local/bin/python3.8" nimble test`
  ##
  ## These are launched via a shell so they can be scripts
  ## as well as actual Python executables

  const sep = ":"
  let pyExes = getEnv(&"NIMPY_PY_EXES", "python2{sep}python3")
  result = pyExes.split(sep)
...
proc calcLibPythons() : seq[string] =
  ...
  ##
  ## `NIMPY_LIBPYTHONS=&"/usr/lib/x86_64-linux-gnu/libpython2.7.so{sep}/usr/lib/x86_64-linux-gnu/libpython3.8.so" nimble test`
  ##
  let libPythons = getEnv("NIMPY_LIBPYTHONS", "")
  result = libPythons.split(sep)
